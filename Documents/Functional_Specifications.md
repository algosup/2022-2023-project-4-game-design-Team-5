...
* [Game](#game)
	...
	* [Functionalities](#functionalities)
		...
		* [Game Over](#game-over)
		* [Menus Interactions](#menus-interactions)
* [Decisions](#decisions)
* [Personas](#personas)
* [Use cases](#use-cases)
...
The gameplay will be the main part of the game. The player will have to go through the levels and reach the coffee cup to sweeten the coffee. Some obstacle will require to be avoided in order to reach to goal. The player will have to use the mechanics of the game to be able to reach the coffee cup. The sticky mechanic will be used to stick to allowed walls and avoid obstacles. Objects will be used to launch yourself from one point to another.

### Level Design

The level design has been decided to be a house with the kitchen as the firsts levels. The levels will take place in different parts of the kitchen such as the worktop, the sink, the coffee table or the sofa. The design will be thought to be as simple as possible to allow the player to focus on the gameplay or the puzzles and not on the level design.
...
The game over screen will be the screen that will appear when the player will die. The death will be caused by the player being too close to a deadly object for too long or a fall from a high place. The menu will allow the player to go back to the mast saving point or to the main menu. In order to keep the lore of the game, the one we thought about, we decided to avoid a game over that would make the game start again with another sugar cube.

### Menus Interactions

Their will be interactions between the menus that the player will have access to. <br>
Here is a list of the possible interactions between the different menus and the game:

![Menus Interactions](./Pictures/menus_interactions.png)

# Decisions

| Decision | Description | Choice | Date |
| -------- | ----------- | ------ | ---- |
| **Supported Platforms** | Choosing the platforms that the game will be available on | Windows and mac | 2023-02-28 |
| **Target Groups/Age** | Choosing the target groups and the age of the players | 7+ | 2023-02-28 |
| **Game Type** | Choosing between the different types of game to find one that every project members accept. | 3D Puzzler | 2023-02-28 |
| **Game Subject** | Finding a subject for the game to have a story and that would make people want to play our game | A sugar cube that has to sweeten a coffee | 2023-02-28 |
